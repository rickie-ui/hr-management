    ...
    <aside class="bg-white w-2/12 h-auto border-0 border-r-2">
      ...
      <ul class="opacity-70">
        <li class="text-md duration-300 hover:text-white hover:bg-[#405189] hover:rounded-sm">
          <a href="{{route('userDash')}}" class="block px-4 py-2 {{ request()->segment(2) == 'dashboard' ? 'bg-[#405189] text-white' : '' }}"><i class="fa fa-dashboard"></i>&ensp; Dashboard</a>
        </li>

        <li class="text-md duration-300 hover:text-white hover:bg-[#405189] hover:rounded-sm">
          <a href="{{route('dayoff')}}" class="block px-4 py-2 {{ request()->segment(2) == 'leave' ? 'bg-[#405189] text-white' : '' }}"><i class="fa fa-list-alt"></i>&ensp; Leave</a>
        </li>
    ...
    <main class="bg-[#F2F2F8] w-10/12 h-auto">
      <header class="flex h-16 w-10/12 justify-between  px-3 bg-white fixed top-0  z-50 shadow-sm">
        ...
        <div class="flex items-center justify-center space-x-2">
         ...
         <a href="{{route('userDash')}}" class="h-10 w-10 p-2 text-md flex justify-center items-center text-gray-500 hover:rounded-full hover:bg-gray-300 hover:text-[#405189]">
              <i class="fa fa-bar-chart"></i>
            </a>

          <a href="{{route('dayoff')}}" class="h-10 w-10 p-2 text-xl flex justify-center items-center text-gray-500 hover:rounded-full hover:bg-gray-300 hover:text-[#405189]">
          ...
          <div class="flex items-center  border-0 border-l-2 px-2  space-x-3">
            ...
            <div class="whitespace-nowrap font-semibold text-black">
                <span class="hover:underline cursor-pointer">{{auth()->user()->fullName}}</span> <br/>
                <span class="font-normal text-gray-400">{{auth()->user()->email}}</span>
            </div>
      </div>
